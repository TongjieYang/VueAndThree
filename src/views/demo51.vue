...
export default {
  ...
  methods: {
      init: function(){
        ...
        var grassTexture = textureLoader.load(grass);
        ...
        //精灵图创建树林效果
        var treeTexture = textureLoader.load(tree);
        for (let i = 0; i < 1000; i++) {
          ...
          var currentScale = Math.random()*100;
          sprite.scale.set(currentScale, currentScale, 1); //// 只需要设置x、y两个分量就可以
          var k1 = Math.random() - 0.5;
          var k2 = Math.random() - 0.5;
          // 设置精灵模型位置，在xoz平面上随机分布
          sprite.position.set(1000 * k1, currentScale/2, 1000 * k2) 
        }
